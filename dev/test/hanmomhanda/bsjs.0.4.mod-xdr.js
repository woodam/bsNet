...
(function(trim){
	...
	url, paramH, paramP, param, xhr, xhrPool, rq, xdr, xdrPool, rqxdr, httpHeader, httpH, http;
	BASE:
	...
	rq = function(x){
		if( x ){
			...
			x.onreadystatechange = null, xhrPool[xhrPool._l++] = x;
		}else return xhrPool._l ? xhrPool[--xhrPool._l] : xhr();
	},
    xdr = W['XDomainRequest'] ? function(){return new XDomainRequest;} : function(){return null;},
    xdrPool = {_l:0},
	rqxdr = function(x){
		if( x ) x.abort(), x.onload = x.onerror = x.onprogress = x.ontimeout = null, xdrPool[xdrPool._l++] = x;
        else return xdrPool._l ? xdrPool[--xdrPool._l] : xdr();
	},
	paramH = [], paramP = [],
	param = function(arg){
		var i, j, k;
	...
	http = function( type, end, url, arg ){
        ...
        if ( url == cpurl && detect.browser == 'ie' && detect.browserVer < 10 && detect.browserVer >= 8 ) {
            rqObj = rqxdr();
            rqObj.onerror = rqObj.ontimeout = function(){rqxdr(rqObj);};
            if( end ) rqObj.onload = function(){
                if( timeId < 0 ) return;
                clearTimeout(timeId), timeId = -1,
                end( rqObj.responseText ), rqxdr(rqObj);
            }, timeId = setTimeout( function(){
                if( timeId > -1 ) timeId = -1, rqxdr(rqObj), end( null, 'timeout' );
            }, timeout );
            rqObj.open( type, url ),
            rqObj.send(arg);
            if( !end ) return i = rqObj.responseText, rqxdr(rqObj), i;
        } else {
            rqObj = rq();
            if( end ) rqObj.onreadystatechange = function(){
                var text, status;
                if( rqObj.readyState != 4 || timeId < 0 ) return;
                clearTimeout(timeId), timeId = -1,
                text = rqObj.status == 200 || rqObj.status == 0 ? rqObj.responseText : null,
                status = text ? rqObj.getAllResponseHeaders() : rqObj.status,
                rq(rqObj), end( text, status );
            }, timeId = setTimeout( function(){
                if( timeId > -1 ) timeId = -1, rq(rqObj), end( null, 'timeout' );
            }, timeout );
            rqObj.open( type, url, end ? true : false ),
            httpH.length = i = 0, j = paramH.length;
            while( i < j ){
                rqObj.setRequestHeader( k = paramH[i++], paramH[i++] );
                if( httpHeader[k] ) httpH[httpH.length] = k;
            }
            for( i in httpHeader ) if( httpH.indexOf(i) == -1 ) j = httpHeader[i], rqObj.setRequestHeader( i, typeof j == 'function' ? j(type) : j );
            rqObj.send(arg);
            if( !end ) return i = rqObj.responseText, rq(rqObj), i;
        }
	},
					...
					if( typeCat ){ 
						if( typeCat < 3 ){
							...
							while( i-- ) self[i] = t1 = t0[i], self['id'+i] = t1.identifier,
								...
								if( typeCat == 1 ) self['mx'+i] = X - self['$x'+i], self['my'+i] = Y - self['$y'+i], self['$x'+i] = X, self['$y'+i] = Y;
